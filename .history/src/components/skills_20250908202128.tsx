...
const CardsCarousel = () => {
  const getVisibleCards = () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      cards.push({ ...categories[index], realIndex: index }); // guardamos el índice real
    }
    return cards;
  };
    ...
    <div className="carousel-container">
      <div className="carousel-cards">
        {getVisibleCards().map((category, idx) => {
          const isActive = idx === 1; // la del centro es la activa
          return (
            <div
              key={`${category.id}-${currentIndex}`}
              className={`card ${isActive ? 'active' : ''}`}
              style={{ border: isActive ? `3px solid ${category.color}` : '3px solid transparent' }}
              onClick={() => setCurrentIndex(category.realIndex)} // <-- aquí hacemos click
            >
